...
void SyntaxAnalyzer::PrintResults(string filelocation)
{
	...
	//this->iTable.Print(filelocation);
}
...
bool SyntaxAnalyzer::IDs()
{
	...
	{		
		...
		case is_insert:
			...
			break;
		case is_reading:
			iTable.GenerateInstruction(i_stdin, sTable.GetAddress(rrr));
			break;
		case is_nothing:
			iTable.GenerateInstruction(i_pushm, sTable.GetAddress(rrr));
			break;
		}
		NextRecord();
		IDsPrime();
...
void SyntaxAnalyzer::StatementListPrime()
{
	if(StatementList())
	{
		logger.Log("<Statement List Prime> ::= <Statement List>");
	}
	else
	{
		// epsilon
...
bool SyntaxAnalyzer::Assign()
{
	if(rrr == Identifier)
	{
		...
		if(rrr == o_equal)
		{
			...
			if(rrr == s_semicolon)
			{
				iTable.GenerateInstruction(i_popm, sTable.GetAddress(save));
				NextRecord();
	...
	if(rrr == k_print)
	{
		...
		if(rrr == s_openbracketround)
		{
			...
			if(rrr == s_closebracketround)
			{
				...
				if(rrr == s_semicolon)
				{
					iTable.GenerateInstruction(i_stdout);
					NextRecord();
...
{  
	if(rrr == k_while)
	{
		int address = iTable.CurrentAddress();
		iTable.GenerateInstruction(i_label);
		NextRecord();
		if(rrr == s_openbracketround)
		{
			NextRecord();
			...
			if(rrr == s_closebracketround)
			{
				iTable.GenerateInstruction(i_jump, address);
				NextRecord();
				Statement();
				// log
...
void SyntaxAnalyzer::Relop()
{
	if(rrr == o_equalequal)
	{
		iTable.GenerateInstruction(i_equ);
		iTable.PushJumpStack();
		iTable.GenerateInstruction(i_jumpz);
		NextRecord();
		logger.Log("<Relop> ::= =");
	}
	...
	{
		iTable.GenerateInstruction(i_equ);
		iTable.GenerateInstruction(i_pushi, 0);
		iTable.GenerateInstruction(i_equ);
		iTable.GenerateInstruction(i_jumpz);
		iTable.PushJumpStack();
		NextRecord();
	...
	{
		iTable.GenerateInstruction(i_grt);
		iTable.GenerateInstruction(i_jumpz);
		iTable.PushJumpStack();
		NextRecord();
		logger.Log("<Relop> ::= >");
	...
	{
		iTable.GenerateInstruction(i_les);
		iTable.PushJumpStack();
		iTable.GenerateInstruction(i_jumpz);
		NextRecord();
		logger.Log("<Relop> ::= <");
	...
	{
		iTable.GenerateInstruction(i_pushi, 1);
		iTable.GenerateInstruction(i_add);
		iTable.GenerateInstruction(i_grt);
		iTable.GenerateInstruction(i_jumpz);
		iTable.PushJumpStack();
		NextRecord();
...
{	
	if(rrr == o_plus)
	{
		NextRecord();
		Term();
		iTable.GenerateInstruction(i_add);// this was i_sub changed it to i_add since its the add operation
		ExpressionPrime();
	...
	{	
		NextRecord();
		Term();
		iTable.GenerateInstruction(i_sub);
		ExpressionPrime();
		logger.Log("<Expression Prime> ::= - <Term> <Expression Prime>");        
	}
	else// if(epsilon())
...
void SyntaxAnalyzer::TermPrime()
{
	...
	if(rrr == o_multiply)
	{
		//iTable.GenerateInstruction(i_mul);
		NextRecord();
		Factor();
		iTable.GenerateInstruction(i_mul);// moved this down to match handout
		logger.Log("<Term Prime> ::= * <Factor> <Term Prime>");// moved this up here since thats how it is in the outline Dr. Choi gave us
		TermPrime();
		//logger.Log("<Term Prime> ::= * <Factor> <Term Prime>");
	}
	else if(rrr == o_divide)
	{
		//iTable.GenerateInstruction(i_div);
		NextRecord();
		Factor();
		iTable.GenerateInstruction(i_div);// moved this down to match the layout of the above if statement
		TermPrime();
		logger.Log("<Term Prime> ::= / <Factor> <Term Prime>");
	}
	else
...
{ 
	if(rrr == o_minus)
	{	
		int recordAddress = sTable.GetAddress(rrr);
		iTable.GenerateInstruction(i_sub, recordAddress);// check this instruction generation
		NextRecord();
		Primary();
		logger.Log("<Factor> ::= - <Primary>");
//		iTable.Print(filelocation);
	}
	else
	{		
		Primary();
...
void SyntaxAnalyzer::Primary()
{
	...
	{
		int integer = atoi(rrr.Lexeme().c_str());
		iTable.GenerateInstruction(i_pushi, integer);
		NextRecord();
